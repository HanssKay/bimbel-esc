...
// Cek apakah user sudah login dan role orangtua
...
// Ambil data anak berdasarkan relasi yang ada di tabel siswa_orangtua
$anak_data = [];
if ($orangtua_id > 0) {
    // Query diperbaiki: ambil data siswa melalui tabel siswa_orangtua
    $sql_anak = "SELECT s.id, s.nama_lengkap, s.kelas 
                 FROM siswa s 
                 INNER JOIN siswa_orangtua so ON s.id = so.siswa_id
                 WHERE so.orangtua_id = ? AND s.status = 'aktif'";
    $stmt_anak = $conn->prepare($sql_anak);
    if ($stmt_anak) {
        $stmt_anak->bind_param("i", $orangtua_id);
        $stmt_anak->execute();
...
// Filter pengumuman (hanya yang aktif/publik)
$current_date = date('Y-m-d H:i:s');
$pengumuman_list = [];

// Query sesuai struktur tabel pengumuman yang baru
$sql = "SELECT p.*, u.full_name as pembuat 
        FROM pengumuman p 
        JOIN users u ON p.dibuat_oleh = u.id 
        WHERE p.status = 'publik' 
        AND (p.target = 'orangtua' OR p.target = 'semua')
...
// Tampilkan jumlah anak untuk debug
// var_dump($anak_data); // Hapus komentar ini untuk debugging jika perlu
?>
    ...
    <div class="desktop-sidebar w-64 bg-blue-800 text-white fixed h-full z-40">
        ...
        <div class="p-4 bg-blue-900">
            ...
            <div class="mt-3 text-sm">
                <p><i class="fas fa-child mr-2"></i> <?php echo count($anak_data); ?> Anak</p>
            </div>
        </div>
        <nav class="mt-4">
                ...
                <div class="flex justify-between">
                    <div class="flex items-center mb-4 md:mb-0">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600">Informasi terbaru dari Bimbel Esc</p>
                        </div>
                    </div>
