...
// ============================================
// ACTION: Input pembayaran baru - SIMPLE VERSION
// ============================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_payment'])) {
    ...
    if (empty($bulan)) $errors[] = "Harap pilih bulan tagihan!";
...
// SESUAIKAN DENGAN STRUKTUR DATABASE BARU
$sql = "SELECT 
    p.*,
    s.nama_lengkap,
    s.kelas,
...
$siswa_result = $conn->query("SELECT 
    ...
    (SELECT GROUP_CONCAT(CONCAT('Kelas: ', ps.jenis_kelas, ' (', ps.tingkat, ')') SEPARATOR ', ') 
     FROM pendaftaran_siswa ps 
     WHERE ps.siswa_id = s.id AND ps.status = 'aktif') as pendaftaran_aktif,
    (SELECT COUNT(*) FROM pendaftaran_siswa ps WHERE ps.siswa_id = s.id AND ps.status = 'aktif') as total_pendaftaran_aktif
FROM siswa s
...
<head>
    ...
    <style>
        ...
        .action-btn { padding: 8px; border-radius: 8px; transition: all 0.2s; }
        ...
        @media (max-width: 767px) { 
            ...
            .detail-value { max-width: 50%; } 
            
            /* Improve mobile table */
            .mobile-table-container { overflow-x: auto; -webkit-overflow-scrolling: touch; }
        ...
        .hint-error { background-color: #FEE2E2; color: #991B1B; }
...
<body class="bg-gray-100">
    ...
    <div class="mobile-header bg-blue-800 text-white w-full fixed top-0 z-30 md:hidden shadow-lg">
        <div class="flex justify-between items-center p-3">
            <div class="flex items-center">
                <button id="menuToggle" class="text-white touch-target w-12 h-12 flex items-center justify-center rounded-full hover:bg-blue-700 active:bg-blue-900 transition-colors">
    ...
    <div id="mobileMenu" class="bg-blue-800 text-white md:hidden">
        <div class="h-full flex flex-col">
            ...
            <div class="p-4 bg-blue-900 flex items-center justify-between">
                ...
                <button id="menuClose" class="text-white touch-target w-10 h-10 flex items-center justify-center rounded-full hover:bg-blue-700 active:bg-blue-900">
    ...
    <div class="md:ml-64">
        ...
        <div class="bg-white shadow px-4 py-3 md:p-4">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                ...
                <div class="mt-2 md:mt-0 text-right">
                    <span class="inline-flex items-center px-3 py-1.5 md:px-3 md:py-2 rounded-md text-xs md:text-sm font-medium bg-blue-100 text-blue-800">
        ...
        <div class="container mx-auto p-3 md:p-6">
            <!-- Messages -->
            <?php if ($success_msg): ?>
            <div class="mb-4 p-3 md:p-4 bg-green-50 border border-green-200 text-green-700 rounded-lg flex items-center">
                <i class="fas fa-check-circle text-green-500 mr-3 text-lg"></i>
                <div><p class="font-medium">Sukses!</p><p class="text-sm"><?= $success_msg ?></p></div>
            </div>
            <?php endif; ?>
            ...
            <div class="mb-4 p-3 md:p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg flex items-center">
                ...
                <div><p class="font-medium">Terjadi Kesalahan!</p><p class="text-sm"><?= $error_msg ?></p></div>
            </div>
            <?php endif; ?>
            ...
            <div class="bg-white rounded-xl shadow p-4 md:p-5 mb-4 md:mb-6">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div class="flex flex-col md:flex-row md:items-center space-y-3 md:space-y-0 mb-3 md:mb-0">
                        ...
                        <div class="md:ml-4">
                            <select id="monthSelect" onchange="changeMonth(this.value)" class="w-full md:w-auto border rounded-lg px-3 md:px-4 py-2.5 bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base">
                                <?php for ($i = 0; $i < 6; $i++): 
                                    $date = date('Y-m', strtotime("-$i months"));
                                    $selected = ($date == $selected_month) ? 'selected' : ''; ?>
                                    <option value="<?= $date ?>" <?= $selected ?>><?= date('F Y', strtotime($date . '-01')) ?></option>
                                <?php endfor; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="w-full md:w-auto">
                        <button onclick="openModal('createModal')" class="w-full md:w-auto px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center justify-center shadow-md text-sm md:text-base">
                ...
                <div class="mt-3 text-xs md:text-sm text-gray-600 bg-gray-50 px-3 md:px-4 py-2 rounded-lg">
                    ...
                    Menampilkan <?= $total_pembayaran ?> pembayaran untuk <span class="font-semibold"><?= date('F Y', strtotime($selected_month . '-01')) ?></span>
                </div>
            </div>
            
            <!-- Stats -->
            <div class="grid grid-cols-2 gap-3 md:gap-6 mb-6 md:mb-8">
                <!--<div class="stat-card bg-white rounded-xl shadow p-3 md:p-5 border-l-4 border-blue-500">-->
                <!--    <div class="flex items-center">-->
                <!--        <div class="p-2 md:p-3 bg-blue-100 text-blue-600 rounded-lg mr-2 md:mr-4">-->
                <!--            <i class="fas fa-file-invoice-dollar text-lg md:text-xl"></i>-->
                <!--        </div>-->
                <!--        <div>-->
                <!--            <p class="text-xs md:text-sm text-gray-600">Total Tagihan</p>-->
                <!--            <h3 class="text-lg md:text-2xl font-bold text-gray-800">Rp <?= number_format($total_tagihan, 0, ',', '.') ?></h3>-->
                <!--        </div>-->
                <!--    </div>-->
                <!--</div>-->
                ...
                <!--<div class="stat-card bg-white rounded-xl shadow p-3 md:p-5 border-l-4 border-red-500">-->
                <!--    <div class="flex items-center">-->
                <!--        <div class="p-2 md:p-3 bg-red-100 text-red-600 rounded-lg mr-2 md:mr-4">-->
                <!--            <i class="fas fa-clock text-lg md:text-xl"></i>-->
                <!--        </div>-->
                <!--        <div>-->
                <!--            <p class="text-xs md:text-sm text-gray-600">Belum Lunas</p>-->
                <!--            <h3 class="text-lg md:text-2xl font-bold text-gray-800"><?= $belum_count ?></h3>-->
                <!--        </div>-->
                <!--    </div>-->
                <!--</div>-->
            ...
            <div class="bg-white rounded-xl shadow overflow-hidden mb-6 md:mb-8">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Orang Tua</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr><td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-base md:text-lg font-medium text-gray-400 mb-1 md:mb-2">Belum ada pembayaran untuk bulan ini</p>
                                    <p class="text-xs md:text-sm text-gray-400 max-w-xs">Gunakan tombol "Input Pembayaran Baru" untuk mencatat pembayaran</p>
                                </div>
                            </td></tr>
                            <?php else: ?>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 md:px-6 py-3 md:py-4">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-2 md:ml-4">
                                            <div class="text-sm font-medium text-gray-900 truncate max-w-[120px] md:max-w-none"><?= htmlspecialchars($p['nama_lengkap']) ?></div>
                                            <div class="text-xs text-gray-500"><?= $p['kelas'] ?></div>
                                            <div class="text-xs text-gray-500 mt-0.5 hidden sm:block"><?= $p['jenis_kelas'] ?> (<?= $p['tingkat'] ?>)</div>
                                            <!-- Mobile tagihan info -->
                                            <div class="text-xs text-gray-700 mt-1 sm:hidden">
                                                <div>Tagihan: <span class="font-semibold">Rp <?= number_format($p['nominal_tagihan'], 0, ',', '.') ?></span></div>
                                                <div>Dibayar: <span class="font-semibold text-green-600">Rp <?= number_format($p['nominal_dibayar'], 0, ',', '.') ?></span></div>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-3 md:px-6 py-3 md:py-4 hidden sm:table-cell">
                                    <div class="space-y-1 md:space-y-2">
                                        ...
                                        <div class="text-xs px-2 py-1 rounded-full inline-block <?= $p['sisa_tagihan'] > 0 ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700' ?>">
                                            <i class="fas fa-<?= $p['sisa_tagihan'] > 0 ? 'exclamation-circle' : 'check-circle' ?> mr-1"></i>
                                ...
                                <td class="px-3 md:px-6 py-3 md:py-4">
                                    <div class="space-y-1 md:space-y-2">
                                        ...
                                        <div class="text-xs text-gray-500 hidden md:flex items-center"><i class="far fa-calendar mr-1"></i><?= $p['tgl_bayar_formatted'] ?></div>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-3 md:px-6 py-3 md:py-4 hidden md:table-cell">
                                    <div class="space-y-1">
                                        <div class="text-sm font-medium text-gray-900 truncate max-w-[150px]"><?= htmlspecialchars($p['nama_ortu'] ?? 'Tidak ada data') ?></div>
                                        <div class="text-xs text-gray-500"><i class="fas fa-phone mr-1"></i><?= $p['no_hp'] ?? '-' ?></div>
                                    </div>
                                </td>
                ...
                <?php if (!empty($pembayaran_list)): ?>
                <div class="px-4 md:px-6 py-3 md:py-4 border-t border-gray-200 bg-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs md:text-sm text-gray-500">
                    <div class="mb-2 sm:mb-0">Menampilkan <span class="font-semibold"><?= count($pembayaran_list) ?></span> pembayaran</div>
    ...
    <div id="createModal" class="modal">
        <div class="modal-content">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-4 md:px-6 py-3 md:py-4 rounded-t-lg">
                <h2 class="text-lg md:text-xl font-bold flex items-center"><i class="fas fa-plus mr-2 md:mr-3"></i> Input Pembayaran Baru</h2>
            </div>
            <form method="POST" class="p-4 md:p-6" id="createForm">
                <input type="hidden" name="create_payment" value="1">
                
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Pilih Siswa: <span class="text-red-500">*</span></label>
                    <select name="siswa_id" id="siswaSelect" class="w-full border rounded-lg px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base" required onchange="showSiswaInfo(this.value)">
                        <option value="">- Pilih Siswa -</option>
                        <?php foreach ($siswa_list as $siswa): 
                            $pendaftaran_info = $siswa['pendaftaran_aktif'] ? $siswa['pendaftaran_aktif'] : 'Belum ada pendaftaran aktif';
                            $total_pendaftaran = $siswa['total_pendaftaran_aktif'] ? intval($siswa['total_pendaftaran_aktif']) : 0;
                        ?>
                        <option value="<?= $siswa['id'] ?>" 
                                data-kelas="<?= htmlspecialchars($siswa['kelas']) ?>" 
                                data-ortu="<?= htmlspecialchars($siswa['nama_ortu'] ?? 'Tidak ada data') ?>" 
                                data-hp="<?= htmlspecialchars($siswa['no_hp'] ?? 'Tidak ada data') ?>"
                                data-email="<?= htmlspecialchars($siswa['email_ortu'] ?? 'Tidak ada data') ?>"
                                data-pendaftaran="<?= htmlspecialchars($pendaftaran_info) ?>"
                                data-total-pendaftaran="<?= $total_pendaftaran ?>">
                            <?= htmlspecialchars($siswa['nama_lengkap']) ?> (<?= $siswa['kelas'] ?>)
                        </option>
                        <?php endforeach; ?>
                    </select>
                    <div id="siswaInfo" class="mt-2 p-3 siswa-info-box rounded-lg hidden">
                        <div class="text-xs text-gray-700 space-y-1">
                            <div class="flex items-center">
                                <i class="fas fa-graduation-cap text-blue-500 mr-2"></i>
                                <span class="font-medium">Kelas:</span> 
                                <span id="infoKelas" class="ml-1">-</span>
                            </div>
                            <div class="flex items-center">
                                <i class="fas fa-user-tie text-green-500 mr-2"></i>
                                <span class="font-medium">Orang Tua:</span> 
                                <span id="infoOrtu" class="ml-1">-</span>
                            </div>
                            <div class="flex items-center">
                                <i class="fas fa-phone text-purple-500 mr-2"></i>
                                <span class="font-medium">No. HP:</span> 
                                <span id="infoHP" class="ml-1">-</span>
                            </div>
                            <div class="flex items-center">
                                <i class="fas fa-envelope text-orange-500 mr-2"></i>
                                <span class="font-medium">Email:</span> 
                                <span id="infoEmail" class="ml-1">-</span>
                            </div>
                            <div class="flex items-center">
                                <i class="fas fa-book-open text-red-500 mr-2"></i>
                                <span class="font-medium">Pendaftaran Aktif:</span> 
                                <span id="infoPendaftaran" class="ml-1">-</span>
                            </div>
                            <div id="pendaftaranWarning" class="text-red-500 font-medium mt-1 hidden">
                                <i class="fas fa-exclamation-triangle mr-1"></i> Siswa tidak memiliki pendaftaran aktif!
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Bulan Tagihan: <span class="text-red-500">*</span></label>
                    <select name="bulan" class="w-full border rounded-lg px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base" required>
                        <?php for ($i = 0; $i < 6; $i++): 
                            $date = date('Y-m', strtotime("+$i months")); ?>
                            <option value="<?= $date ?>" <?= ($date == $current_month) ? 'selected' : '' ?>><?= date('F Y', strtotime($date . '-01')) ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                ...
                <!-- FIELD STATUS -->
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Status Pembayaran: <span class="text-red-500">*</span></label>
                    <select name="status" id="statusSelect" class="w-full border rounded-lg px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base" required onchange="handleStatusChange(this.value)">
                ...
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Nominal Tagihan: <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span class="text-gray-500">Rp</span></div>
                        <input type="text" name="nominal_tagihan" id="nominalTagihan" value="<?= number_format($NOMINAL_DEFAULT, 0, ',', '.') ?>" class="w-full border rounded-lg pl-10 pr-3 py-2.5 md:py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base" required oninput="formatCurrency(this)">
                    </div>
                </div>
                
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Nominal Dibayar: <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span class="text-gray-500">Rp</span></div>
                        <input type="text" name="nominal_dibayar" id="nominalDibayar" value="<?= number_format($NOMINAL_DEFAULT, 0, ',', '.') ?>" class="w-full border rounded-lg pl-10 pr-3 py-2.5 md:py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base" required oninput="formatCurrency(this)">
                    </div>
                ...
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Metode Bayar:</label>
                    <select name="metode_bayar" id="metodeBayar" class="w-full border rounded-lg px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base">
                ...
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Keterangan:</label>
                    <textarea name="keterangan" rows="2" class="w-full border rounded-lg px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base" placeholder="Catatan..."></textarea>
                </div>
                
                <div class="flex justify-end space-x-2 md:space-x-3 mt-4 md:mt-6">
                    <button type="button" onclick="closeModal('createModal')" class="px-4 md:px-5 py-2 md:py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50 font-medium text-sm md:text-base">Batal</button>
                    <button type="submit" class="px-4 md:px-5 py-2 md:py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium flex items-center text-sm md:text-base">
    ...
    <div id="updateModal" class="modal">
        <div class="modal-content">
            <div class="bg-gradient-to-r from-purple-600 to-purple-700 text-white px-4 md:px-6 py-3 md:py-4 rounded-t-lg">
                <h2 class="text-lg md:text-xl font-bold flex items-center"><i class="fas fa-edit mr-2 md:mr-3"></i> Update Pembayaran</h2>
            </div>
            <form method="POST" class="p-4 md:p-6" id="updateForm">
                <input type="hidden" name="pembayaran_id" id="updatePaymentId">
                <input type="hidden" name="update_payment" value="1">
                
                <div class="mb-3 md:mb-4 p-3 bg-blue-50 rounded-lg">
                    <div class="text-sm font-medium text-gray-900" id="updateNamaSiswa"></div>
                    <div class="text-xs text-gray-600">Tagihan: Rp <span id="updateTagihanAmount" class="font-semibold">0</span></div>
                    <div class="text-xs text-gray-600 mt-1" id="updateBulanTagihan"></div>
                </div>
                
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Status: <span class="text-red-500">*</span></label>
                    <select name="status" id="updateStatusSelect" class="w-full border rounded-lg px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-sm md:text-base" required onchange="handleUpdateStatusChange(this.value)">
                        <?php foreach ($STATUS_PEMBAYARAN as $value => $label): ?>
                        <option value="<?= $value ?>"><?= $label ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="mb-4 md:mb-5">
                    ...
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span class="text-gray-500">Rp</span></div>
                        <input type="text" name="nominal_dibayar" id="updateNominalDibayar" class="w-full border rounded-lg pl-10 pr-3 py-2.5 md:py-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-sm md:text-base" required oninput="formatCurrency(this)">
                    </div>
                </div>
                
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Metode Bayar:</label>
                    <select name="metode_bayar" id="updateMetodeBayar" class="w-full border rounded-lg px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-sm md:text-base">
                ...
                <div class="mb-4 md:mb-5">
                    <label class="block text-gray-700 mb-1 md:mb-2 font-medium text-sm md:text-base">Keterangan:</label>
                    <textarea name="keterangan" id="updateKeterangan" rows="2" class="w-full border rounded-lg px-3 md:px-4 py-2.5 md:py-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-sm md:text-base"></textarea>
                </div>
                
                <div class="flex justify-end space-x-2 md:space-x-3 mt-4 md:mt-6">
                    <button type="button" onclick="closeModal('updateModal')" class="px-4 md:px-5 py-2 md:py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50 font-medium text-sm md:text-base">Batal</button>
                    <button type="submit" class="px-4 md:px-5 py-2 md:py-2.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 font-medium flex items-center text-sm md:text-base">
    ...
    <div id="detailModal" class="modal">
        <div class="modal-content">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-4 md:px-6 py-3 md:py-4 rounded-t-lg">
                <h2 class="text-lg md:text-xl font-bold flex items-center"><i class="fas fa-info-circle mr-2 md:mr-3"></i> Detail Pembayaran</h2>
            </div>
            <div class="p-4 md:p-6">
                ...
                <div class="mb-4 md:mb-6">
                    ...
                    <div class="space-y-2 md:space-y-3">
                        <div class="detail-item">
                            ...
                            <span id="detailNominalTagihan" class="detail-value text-xs md:text-sm font-semibold"></span>
                        </div>
                        <div class="detail-item">
                ...
                <!--    <h4 class="font-medium text-gray-700 mb-2 md:mb-3 flex items-center text-sm md:text-base">-->
                <!--        <i class="fas fa-history mr-2 text-gray-500"></i>Informasi Sistem-->
                <!--    </h4>-->
                <!--    <div class="space-y-1 md:space-y-2">-->
                <!--        <div class="flex justify-between items-center">-->
                <!--            <span class="text-gray-600 text-xs md:text-sm">Dibuat oleh:</span>-->
                <!--            <span id="detailDibuatOleh" class="text-xs md:text-sm font-medium"></span>-->
                <!--        </div>-->
                <!--        <div class="flex justify-between items-center">-->
                <!--            <span class="text-gray-600 text-xs md:text-sm">Dibuat pada:</span>-->
                <!--            <span id="detailDibuatPada" class="text-xs md:text-sm"></span>-->
                <!--        </div>-->
                <!--        <div class="flex justify-between items-center">-->
                <!--            <span class="text-gray-600 text-xs md:text-sm">Diperbarui oleh:</span>-->
                <!--            <span id="detailDiperbaruiOleh" class="text-xs md:text-sm font-medium"></span>-->
                <!--        </div>-->
                <!--        <div class="flex justify-between items-center">-->
                <!--            <span class="text-gray-600 text-xs md:text-sm">Diperbarui pada:</span>-->
                <!--            <span id="detailDiperbaruiPada" class="text-xs md:text-sm"></span>-->
                <!--        </div>-->
                <!--    </div>-->
                <!--</div>-->
                
                <div class="flex justify-end">
                    <button type="button" onclick="closeModal('detailModal')" class="px-4 md:px-5 py-2 md:py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium transition-colors duration-150 text-sm md:text-base">
    ...
    function openModal(modalId) {
        ...
        // Reset form create modal saat dibuka
    ...
    // Fungsi untuk menampilkan info siswa yang dipilih
    function showSiswaInfo(siswaId) {
        ...
        const pendaftaranWarning = document.getElementById('pendaftaranWarning');
        
        if (siswaId && selectedOption) {
            const kelas = selectedOption.getAttribute('data-kelas') || '-';
            const ortu = selectedOption.getAttribute('data-ortu') || '-';
            const hp = selectedOption.getAttribute('data-hp') || '-';
            const email = selectedOption.getAttribute('data-email') || '-';
            const pendaftaran = selectedOption.getAttribute('data-pendaftaran') || 'Tidak ada pendaftaran aktif';
            const totalPendaftaran = parseInt(selectedOption.getAttribute('data-total-pendaftaran') || '0');
            
            document.getElementById('infoKelas').textContent = kelas;
            document.getElementById('infoOrtu').textContent = ortu;
            document.getElementById('infoHP').textContent = hp;
            document.getElementById('infoEmail').textContent = email;
            document.getElementById('infoPendaftaran').textContent = pendaftaran;
            
            // Tampilkan warning jika tidak ada pendaftaran aktif
            if (totalPendaftaran === 0) {
                pendaftaranWarning.classList.remove('hidden');
            } else {
                pendaftaranWarning.classList.add('hidden');
            }
    ...
    // Fungsi untuk mengatur form create berdasarkan status
    function handleStatusChange(status) {
        ...
        metodeBayar.classList.remove('border-yellow-300', 'border-green-300');
        ...
        switch(status) {
            case 'lunas':
                ...
                metodeBayar.classList.add('border-yellow-300');
            ...
            case 'dibebaskan':
                ...
                metodeBayar.value = '';
                metodeBayar.required = false;
                metodeBayar.classList.remove('border-yellow-300');
    ...
    // Fungsi untuk mengatur form update berdasarkan status
    function handleUpdateStatusChange(status) {
        const nominalDibayar = document.getElementById('updateNominalDibayar');
        const metodeBayar = document.getElementById('updateMetodeBayar');
        const tagihanAmount = document.getElementById('updateTagihanAmount').textContent.replace(/\./g, '');
        
        switch(status) {
            case 'lunas':
                // Jika pilih lunas, set nominal dibayar = tagihan
                nominalDibayar.value = formatNumber(tagihanAmount);
                metodeBayar.required = true;
                metodeBayar.classList.add('border-yellow-300');
                break;
                
            case 'dibebaskan':
                // Jika pilih dibebaskan, set nominal dibayar = 0
                nominalDibayar.value = '0';
                metodeBayar.value = '';
                metodeBayar.required = false;
                metodeBayar.classList.remove('border-yellow-300');
    ...
    function openDetailModal(paymentData) {
        ...
        // // Sistem info
        // document.getElementById('detailDibuatOleh').textContent = paymentData.dibuat_oleh_nama || '-';
        // document.getElementById('detailDibuatPada').textContent = paymentData.dibuat_formatted || '-';
        // document.getElementById('detailDiperbaruiOleh').textContent = paymentData.diperbarui_oleh_nama || '-';
        // document.getElementById('detailDiperbaruiPada').textContent = paymentData.diperbarui_formatted || '-';
        
        openModal('detailModal');
    }
    
    // Open Update Modal
    function openUpdateModal(paymentData) {
        document.getElementById('updatePaymentId').value = paymentData.id;
        document.getElementById('updateNamaSiswa').textContent = paymentData.nama_lengkap + ' - ' + paymentData.jenis_kelas;
        document.getElementById('updateTagihanAmount').textContent = new Intl.NumberFormat('id-ID').format(paymentData.nominal_tagihan);
        document.getElementById('updateBulanTagihan').textContent = 'Bulan: ' + formatMonth(paymentData.bulan_tagihan);
        document.getElementById('updateStatusSelect').value = paymentData.status;
        document.getElementById('updateNominalDibayar').value = paymentData.nominal_dibayar ? new Intl.NumberFormat('id-ID').format(paymentData.nominal_dibayar) : '';
        document.getElementById('updateMetodeBayar').value = paymentData.metode_bayar || '';
        document.getElementById('updateKeterangan').value = paymentData.keterangan || '';
        
        // Panggil sekali untuk set initial state
        handleUpdateStatusChange(paymentData.status);
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape') {
            ...
            // Juga tutup mobile menu jika terbuka
            if (document.getElementById('mobileMenu').classList.contains('menu-open')) {
                document.getElementById('mobileMenu').classList.remove('menu-open');
                document.getElementById('menuOverlay').classList.remove('active');
                document.body.style.overflow = 'auto';
            }
    ...
    // ============================================
    // FIX: Event listeners untuk form submit
    // ============================================
    document.addEventListener('DOMContentLoaded', function() {
        // Format input nominal saat form create submit
        const createForm = document.getElementById('createForm');
        if (createForm) {
            createForm.addEventListener('submit', function() {
                // Format nominal tagihan dan nominal dibayar
                ...
                console.log('Submitting create form with status:', statusSelect ? statusSelect.value : 'none');
            });
        }
        
        // Format input nominal saat form update submit
        ...
        const createBtn = document.querySelector('button[onclick*="createModal"]');
        if (createBtn) {
