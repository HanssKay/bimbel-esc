...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['tambah_staff'])) {
    ...
    if (empty($errors)) {
        ...
        if ($insert_stmt) {
            $insert_stmt->bind_param("ssssssi", $username, $hashed_password, $email, 
                                    $full_name_input, $phone, $address, $is_active);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_staff'])) {
    ...
    if (empty($errors)) {
        if ($update_password) {
            ...
            if ($update_stmt) {
                $update_stmt->bind_param("ssssssii", $username, $hashed_password, $email, 
                                        $full_name_input, $phone, $address, $is_active, $staff_id);
            }
        } else {
            ...
            $update_stmt = $conn->prepare($update_sql);
            if ($update_stmt) {
                $update_stmt->bind_param("sssssii", $username, $email, $full_name_input, 
                                        $phone, $address, $is_active, $staff_id);
            }
        }
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                ...
                <div class="bg-white border border-gray-200 rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            <p class="text-sm font-medium text-gray-600">Login 7 Hari</p>
                            <p class="text-lg font-semibold text-gray-900"><?php echo $statistik['aktif_minggu_ini']; ?></p>
                        </div>
                    </div>
                </div>
            </div>
